...
try:
    ...
    logger.info("✅ PC metadata loaded from planetary-computer-tasks repository - single source of truth active")
except ImportError as e:
    PC_METADATA_AVAILABLE = False
    logger.warning(f"⚠️ PC metadata loader not available: {e}")
...
# ============================================================================
# 🗺️ GEOCODING PLUGIN - Azure Maps Tool for AI Agent Function Calling
# ============================================================================
...
class GeocodingPlugin:
    ...
    def __init__(self):
        ...
        if not self.azure_maps_key:
            logger.warning("⚠️ AZURE_MAPS_SUBSCRIPTION_KEY not set - geocoding tool will be unavailable")
                ...
                async with session.get(url, params=params, timeout=timeout) as response:
                    if response.status == 200:
                        ...
                        if data.get("results") and len(data["results"]) > 0:
                            ...
                            logger.info(f"🗺️ Azure Maps geocode: '{location_name}' → {display_name} ({country}), bbox={bbox}")
                            ...
                            logger.warning(f"🗺️ Azure Maps: No results for '{location_name}'")
                            return json.dumps({
                        ...
                        logger.error(f"🗺️ Azure Maps API error {response.status}: {error_text}")
                        return json.dumps({
                            "error": f"Azure Maps API error: {response.status}",
                            "location_name": location_name
                        })
        except asyncio.TimeoutError:
            logger.error(f"🗺️ Azure Maps timeout for '{location_name}'")
            return json.dumps({"error": "Geocoding request timed out", "location_name": location_name})
        except Exception as e:
            logger.error(f"🗺️ Azure Maps exception for '{location_name}': {e}")
    ...
    async def azure_maps_reverse_geocode(self, latitude: float, longitude: float) -> str:
        ...
        try:
                ...
                async with session.get(url, params=params, timeout=timeout) as response:
                    if response.status == 200:
                        ...
                        if features and len(features) > 0:
                            ...
                            logger.info(f"🗺️ Azure Maps reverse geocode: ({latitude}, {longitude}) → {name}, {country}")
                            return json.dumps(result)
                        else:
                            logger.warning(f"🗺️ Azure Maps: No results for ({latitude}, {longitude})")
                            return json.dumps({
                                "error": "Location not found",
                        ...
                        error_text = await response.text()
                        logger.error(f"🗺️ Azure Maps reverse geocode API error {response.status}: {error_text}")
                        return json.dumps({
                            "error": f"Azure Maps API error: {response.status}",
                            "name": f"Location ({latitude:.4f}, {longitude:.4f})",
                            "coordinates": {"latitude": latitude, "longitude": longitude}
            ...
            logger.error(f"🗺️ Azure Maps reverse geocode timeout for ({latitude}, {longitude})")
            return json.dumps({
            ...
            logger.error(f"🗺️ Azure Maps reverse geocode exception for ({latitude}, {longitude}): {e}")
            return json.dumps({
                "error": str(e),
                "name": f"Location ({latitude:.4f}, {longitude:.4f})",
                "coordinates": {"latitude": latitude, "longitude": longitude}
...
class SemanticQueryTranslator:
    ...
    def __init__(self, azure_openai_endpoint: str, azure_openai_api_key: str, model_name: str, azure_credential=None):
        ...
        # 🤖 Model override for runtime model switching
        self._model_override = None
        
        logger.info("✓ Using consolidated EnhancedLocationResolver (Azure Maps → Nominatim → Azure OpenAI)")
        logger.info(f"✓ Authentication mode: {'Managed Identity' if azure_credential else 'API Key'}")
    ...
    def set_model(self, model_name: str) -> None:
        ...
        if model_name and model_name != self._model_override:
            logger.info(f"🤖 Model override set: {self._model_override or self.model_name} → {model_name}")
    ...
    def determine_stac_source(self, query: str, entities: Dict[str, Any]) -> str:
        ...
        logger.info(f"🛰️ VEDA disabled - routing all queries to Planetary Computer STAC: {query}")
    ...
    def build_veda_stac_query(self, entities: Dict[str, Any], bbox: Optional[List[float]]) -> Dict[str, Any]:
        ...
        if not veda_collections:
            ...
            if any(term in query_text for term in ["jrc", "surface water", "global surface water"]):
                ...
                logger.info(f"🌊 JRC detected - should use Planetary Computer, not VEDA")
                ...
                logger.info(f"🌡️ SST detected - should use Planetary Computer, not VEDA")
                return None  # Will fall through to PC handler
...
**⚠️ FOLLOW-UP DETECTION (HIGHEST PRIORITY):**
...
"""
    ...
    async def _ensure_kernel_initialized(self):
        ...
        try:
            ...
            if self.azure_credential and not self.azure_openai_api_key:
                # Use managed identity - get fresh token
                logger.info(f"   🔑 Using managed identity authentication...")
                token_response = self.azure_credential.get_token("https://cognitiveservices.azure.com/.default")
                ad_token = token_response.token
                logger.info(f"   ✅ Obtained Azure AD token (expires: {token_response.expires_on})")
            ...
            if ad_token:
                ...
                logger.info(f"   ✅ AzureChatCompletion service created with managed identity ({active_model})")
            else:
                azure_chat_service = AzureChatCompletion(
                    deployment_name=active_model,
                ...
                logger.info(f"   ✅ AzureChatCompletion service created with API key ({active_model})")
            
            self.kernel.add_service(azure_chat_service)
            logger.info(f"   ✅ {active_model} service added to kernel")
            ...
            try:
                ...
                logger.info(f"   ✅ AzureChatCompletion service created (GPT-4o)")
                
                self.kernel.add_service(azure_chat_service_4o)
                logger.info(f"   ✅ GPT-4o service added to kernel for performance optimization")
            except Exception as e:
                logger.warning(f"   ⚠️ Failed to add GPT-4o secondary service: {e}")
                # Not fatal - will continue with primary GPT-4o service
            
            # ================================================================
            # 🗺️ REGISTER GEOCODING PLUGIN - Azure Maps tool for function calling
            # ================================================================
            ...
            try:
                ...
                logger.info(f"   ✅ Geocoding plugin registered (azure_maps_geocode tool available)")
            except Exception as e:
                logger.warning(f"   ⚠️ Failed to register geocoding plugin: {e}")
            ...
            auth_mode = "Managed Identity" if ad_token else "API Key"
            logger.info(f"   ✓ Semantic Kernel initialized with {self.model_name} at {base_url} ({auth_mode}, API v2024-10-21)")
    ...
    async def collection_mapping_agent(self, query: str) -> List[str]:
        ...
        # ========================================================================
        # ⚡ PERFORMANCE OPTIMIZATION: Fast keyword matching before LLM
        # ========================================================================
        ...
        if keyword_matched:
            logger.info(f"⚡ FAST KEYWORD MATCH: {keyword_matched} (skipped LLM - saved ~1.2s)")
...
Format: ["collection-id"]"""
            ...
            logger.info(f"✅ AGENT 1: Execution settings created (model=gpt-4o, temp=0.3, max_tokens=300)")
            ...
            if PC_METADATA_AVAILABLE:
                ...
                logger.info(f"🔍 AGENT 1: Using PC metadata as source of truth ({len(KNOWN_COLLECTIONS)} collections from PC repository)")
            else:
            ...
            logger.info("=" * 80)
            logger.info(f"🤖 STEP 3: COLLECTION MAPPER (AGENT 1) - OUTPUT")
            logger.info(f"🆔 SESSION: {self.current_session_id}")
            logger.info(f"📥 INPUT Query: '{query}'")
            logger.info(f"📤 OUTPUT Collections: {valid_collections}")
            logger.info(f"📊 Collection Count: {len(valid_collections)}")
            # NOTE: Typically returns exactly 1 collection. Exception: HLS returns 2 (hls2-l30, hls2-s30)
            if len(valid_collections) == 1:
                logger.info(f"   ✅ Single collection selected (standard behavior)")
            elif len(valid_collections) == 2 and all('hls' in c.lower() for c in valid_collections):
                logger.info(f"   ✅ HLS pair selected (expected: hls2-l30 + hls2-s30 = same product family)")
            else:
                logger.warning(f"   ⚠️ Multiple collections returned - unusual (should typically be 1 or HLS pair)")
    ...
    def _fast_collection_keyword_match(self, query: str) -> Optional[List[str]]:
        """
        ⚡ FAST KEYWORD MATCHING - Single source of truth for collection selection
        ...
        if any(k in query_lower for k in ["hls", "harmonized landsat sentinel", "harmonized landsat"]):
            logger.info(f"⚡ HLS DETECTED in query")
        ...
        if "modis" in query_lower:
            logger.info(f"⚡ MODIS DETECTED in query")
        ...
        if any(k in query_lower for k in ["sea surface temperature", "sst", "ocean temperature"]):
            logger.info(f"⚡ SST DETECTED → noaa-cdr-sea-surface-temperature-whoi")
            return ["noaa-cdr-sea-surface-temperature-whoi"]
        
        # Chloris biomass
        if any(k in query_lower for k in ["chloris", "biomass"]):
            logger.info(f"⚡ CHLORIS DETECTED → chloris-biomass")
            return ["chloris-biomass"]
        
        # USDA Cropland Data Layer
        if any(k in query_lower for k in ["usda", "cdl", "cropland data layer", "cropland"]):
            logger.info(f"⚡ USDA CDL DETECTED → usda-cdl")
            return ["usda-cdl"]
        
        # Microsoft Buildings
        if any(k in query_lower for k in ["building", "buildings", "ms-buildings"]):
            logger.info(f"⚡ BUILDINGS DETECTED → ms-buildings")
            return ["ms-buildings"]
        
        # NAIP
        if "naip" in query_lower:
            logger.info(f"⚡ NAIP DETECTED → naip")
            return ["naip"]
        
        # Copernicus DEM
        if any(k in query_lower for k in ["cop-dem", "copernicus dem"]):
            logger.info(f"⚡ COPERNICUS DEM DETECTED → cop-dem-glo-30")
            return ["cop-dem-glo-30"]
        
        # 3DEP
        if any(k in query_lower for k in ["3dep", "lidar"]):
            logger.info(f"⚡ 3DEP DETECTED → 3dep-lidar-hag")
        ...
        for keyword in sorted(GENERIC_INTENT_KEYWORDS.keys(), key=len, reverse=True):
            if keyword in query_lower:
                ...
                logger.info(f"⚡ GENERIC KEYWORD MATCH: '{keyword}' → {collections}")
                return collections
        
        # No match - let LLM handle truly ambiguous queries
        logger.info("🤖 No keyword match - deferring to LLM")
        return None
    
    def _fast_location_keyword_match(self, query: str) -> Optional[str]:
        """
        ⚡ FAST LOCATION EXTRACTION - Check for known locations BEFORE calling GPT
        ...
        try:
            from location_resolver import EnhancedLocationResolver
            hardcoded_locations = list(EnhancedLocationResolver.HARDCODED_US_LOCATIONS.keys())
            logger.debug(f"⚡ Loaded {len(hardcoded_locations)} locations from EnhancedLocationResolver")
        except Exception as e:
            logger.warning(f"⚠️ Could not load locations from location_resolver: {e}")
        ...
        for location in hardcoded_locations_sorted:
            ...
            for pattern in patterns:
                if re.search(pattern, query_lower):
                    ...
                    logger.info(f"⚡ FAST LOCATION MATCH: Found '{display_name}' in query")
                    return display_name
    ...
    def _build_comprehensive_collection_catalog(self) -> str:
        ...
        try:
            ...
            if not collections_file.exists():
                logger.warning(f"⚠️ pc_rendering_config.json not found at {collections_file}")
                return self._get_hardcoded_collection_catalog()
            
            logger.info(f"📂 Loading collection catalog from {collections_file}")
            ...
            catalog_text = "\n".join(catalog)
            logger.info(f"✅ Built catalog from pc_rendering_config.json ({len(catalog_text)} characters, {len(collections_dict)} collections)")
            return catalog_text
            
        except Exception as e:
            logger.error(f"❌ Failed to load pc_rendering_config.json: {e}")
            logger.warning("⚠️ Falling back to hardcoded catalog")
            return self._get_hardcoded_collection_catalog()
    ...
    def _get_hardcoded_collection_catalog(self) -> str:
        ...
        """
        logger.error("❌ CRITICAL: pc_rendering_config.json is missing! Using emergency fallback.")
        logger.error("❌ This fallback is incomplete and should not be used in production.")
        
        return """EMERGENCY FALLBACK - LIMITED COLLECTIONS:
================================================================================
⚠️  WARNING: pc_rendering_config.json is missing!
⚠️  Only essential collections available in this fallback mode.
⚠️  Many collections (including 3DEP LiDAR) are NOT available.
================================================================================
...
⚠️  For full collection catalog, ensure pc_rendering_config.json is present.
================================================================================"""
    ...
    def _select_collections_fallback(self, query: str) -> List[str]:
        ...
        # Default to HLS (Harmonized Landsat Sentinel) - best general-purpose imagery
        # HLS provides 30m resolution harmonized data combining Landsat 8-9 and Sentinel-2
        logger.info(f"🔄 No specific collection detected - defaulting to HLS")
        return ["hls2-l30", "hls2-s30"]
    ...
    async def build_stac_query_agent(self, query: str, collections: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if is_usda_cdl:
                ...
                logger.info(f"🌾 USDA CDL: Added filter for 'cropland' type items (required for render config)")
                print(f"✅ DEBUG: USDA CDL type filter applied to STAC query")
            
            # STEP 6: Resolve location to bbox if present
            # ========================================================================
            # 🚨 LOCATION REQUIREMENT: Location is mandatory for STAC queries
            # ========================================================================
            ...
            if location_name:
                ...
                if bbox and self._validate_bbox(bbox):
                    ...
                    # ====================================================================
                    # 📍 LOCATION AGENT OUTPUT LOGGING
                    # ====================================================================
                    logger.info("=" * 80)
                    logger.info(f"🤖 AGENT 2.1 (LOCATION EXTRACTION) - OUTPUT")
                    logger.info(f"🆔 SESSION: {self.current_session_id}")
                    logger.info(f"📥 INPUT Query: '{query}'")
                    logger.info(f"📤 OUTPUT Location Name: '{location_name}'")
                    logger.info(f"📍 OUTPUT Bounding Box: {bbox}")
                    logger.info(f"🗺️  Coordinates Breakdown:")
                    ...
                    logger.info(f"📐 Area width: {bbox[2] - bbox[0]:.6f}° longitude")
                    logger.info(f"📐 Area height: {bbox[3] - bbox[1]:.6f}° latitude")
                    logger.info("=" * 80)
                    
                    # ================================================================
                    # 📊 DYNAMIC TILE LIMIT - Calculate based on bbox size
                    # ================================================================
                    ...
                    if bbox_area_degrees > 50:  # Large country (e.g., Greece ~71 sq deg)
                        ...
                        logger.info(f"🌍 LARGE COUNTRY detected: {bbox_area_degrees:.1f} sq deg → limit={dynamic_limit}")
                    ...
                    logger.info(f"📊 DYNAMIC TILE LIMIT CALCULATION:")
                    ...
                    print(f"📊 Dynamic limit: {dynamic_limit} tiles (based on {bbox_area_degrees:.1f} sq deg area)")
                ...
                logger.warning(f"⚠️ LOCATION REQUIRED: No location found in query '{query}'")
                print(f"❌🚨 DEBUG: STEP 6 FAILED - No location in query (REQUIRED)")
                
                # Return special error code that caller can detect
                raise ValueError("LOCATION_REQUIRED")
            
            # ========================================================================
            # 📊 FINAL STAC QUERY SUMMARY - Critical for debugging
            # ========================================================================
            logger.info("=" * 80)
            logger.info("🎯 AGENT 2 COMPLETED - FINAL STAC QUERY BUILT")
            logger.info("=" * 80)
            ...
            logger.info(f"📍 Location Name: {stac_query.get('location_name', 'NONE')}")
            logger.info(f"☁️  Cloud Filter: {stac_query.get('query', {})}")
            logger.info(f"🔢 Limit: {stac_query.get('limit', 'default')}")
            logger.info(f"🔎 Full Query JSON: {json.dumps(stac_query, indent=2)}")
            logger.info("=" * 80)
    ...
    async def location_extraction_agent(self, query: str) -> Dict[str, Any]:
        ...
        # ========================================================================
        # ⚡ FAST KEYWORD EXTRACTION - Check for known locations BEFORE calling GPT
        # ========================================================================
        ...
        if fast_location:
            logger.info(f"⚡ FAST LOCATION MATCH: '{fast_location}' (skipped GPT - saved ~1.2s)")
            return {
...
Return only the JSON object. No explanations or additional text."""
        ...
        try:
            ...
            logger.info("🔧 Function calling enabled - GPT-4o can use azure_maps_geocode tool")

            # Execute using simplified invoke_prompt (no template needed with f-string)
            arguments = KernelArguments(settings=execution_settings)
            
            logger.info(f"🔍 Sending location extraction prompt to GPT-4o...")
            logger.info(f"🔧 Using GPT-4o (temp=0.3, max_tokens=300) for structured entity extraction")
...
- Prioritize quality over quantity"""
                ...
                try:
                    ...
                    for service_id, service in self.kernel._services.items():
                        if hasattr(service, 'ai_model_id'):
                            ...
                            if 'gpt-4o' in model_id.lower() or 'gpt4o' in model_id.lower():
                                ...
                                logger.info(f"✅ Using GPT-4o for tile selection: {model_id}")
                                break
                    ...
                    logger.warning(f"⚠️ Could not check for GPT-4o service: {e}, using default")
    ...
    async def _rule_based_tile_selector(
        ...
        try:
            ...
            if len(selected) < 5 and len(selected_time_tiles) > len(selected):
                logger.info(f"   ⚠️ Only {len(selected)} tiles selected, adding more for coverage")
                for tile in selected_time_tiles:
                    if len(selected) >= tile_limit:
                        break
            ...
            logger.info(f"✅ AGENT 3 FALLBACK: Selected {len(selected)} tiles for FULL COVERAGE")
            if selected:
    ...
    async def _build_stac_parameters(self, query: str, collections: List[str], entities: Optional[Dict[str, Any]] = None, datetime_range: Optional[str] = None) -> Dict[str, Any]:
        ...
        if datetime_range:
            ...
            logger.info("=" * 80)
            logger.info(f"📅 DATETIME FILTER - ADDED TO STAC QUERY")
            logger.info("=" * 80)
            logger.info(f"📤 SENDING TO STAC API: datetime={datetime_range}")
            if '/' in datetime_range:
                start, end = datetime_range.split('/')
                logger.info(f"   ├─ Start Date: {start}")
                logger.info(f"   └─ End Date:   {end}")
            logger.info("=" * 80)
            print(f"✅ Datetime added to STAC query: {datetime_range}")
        else:
            logger.info("=" * 80)
        ...
        logger.info(f"✅ Default query limit: 50 (will be adjusted based on bbox size)")
        print(f"✅ Default limit: 50 tiles (dynamic adjustment after bbox resolution)")
    ...
    def _resolve_temporal_to_datetime(self, entities: Optional[Dict[str, Any]], collections: List[str]) -> Optional[str]:
        ...
        if relative == "recent":
            # 🔍 Use 6-month lookback for optical imagery (HLS/Sentinel/Landsat have ingestion lag)
            ...
            logger.info(f"🔍 DEBUG: Using {lookback_days}-day lookback for 'recent' (optical imagery lag)")
            print(f"🔍 DEBUG: Case 4 (Relative 'recent') - last {lookback_days} days for optical data")
            ...
            logger.info(f"🗓️ Resolved temporal: Recent (last {lookback_days} days) → {datetime_range}")
    ...
    async def datetime_translation_agent(
        ...
        # ========================================================================
        logger.info("=" * 80)
        logger.info(f"🤖 AGENT 2.2 (DATETIME TRANSLATION) - START")
        logger.info("=" * 80)
        logger.info(f"📥 INPUT - User Query: '{query}'")
        logger.info(f"📥 INPUT - Collections: {collections}")
        logger.info(f"📥 INPUT - Mode: {mode}")
        from datetime import datetime
        logger.info(f"📅 CONTEXT - Current Date: {datetime.now().strftime('%Y-%m-%d')}")
        logger.info(f"📅 CONTEXT - Current Year: {datetime.now().year}")
        logger.info("=" * 80)
        print(f"🤖 DEBUG: ========== DATETIME AGENT START ==========")
        ...
        # ✅ CRITICAL FIX 2: Detect if user explicitly mentioned a date/year/month
        ...
        if has_explicit_date:
            logger.info("🗓️ User explicitly mentioned date/time → FORCING datetime filter")
            print(f"🗓️ DEBUG: Explicit date detected in query - will apply datetime filter")
        ...
        # ✅ ONLY skip datetime if ALL collections are composite AND user didn't explicitly specify a date
        if not has_non_composite and not has_explicit_date:
            logger.info("🗓️ All composite collections AND no explicit date → No datetime filter")
            print("🗓️ DEBUG: Composite collections with no explicit date - no datetime needed")
            return None
        ...
        try:
            ...
            logger.info(f"🔧 AGENT 2.2: Using GPT-4o (temp=0.3, max_tokens={300 if mode == 'comparison' else 150})")
...
⚠️⚠️⚠️ CRITICAL: ALWAYS use the EXACT year mentioned in the query! ⚠️⚠️⚠️
...
❌ Query: "fire data for California in July 2024" → Response: "2025-07-01/2025-07-31" (WRONG! Changed 2024 to 2025!)
❌ Query: "from 2024" → Response: "2025-01-01/2025-12-31" (WRONG! Changed 2024 to 2025!)
...
Return ONLY a JSON object with "before", "after", and "explanation". If ambiguous, include "needs_clarification", "suggestion", and fallback dates."""
    ...
    def _parse_single_datetime_response(self, response: Dict[str, Any]) -> Optional[str]:
        ...
        # ========================================================================
        logger.info("=" * 80)
        logger.info(f"🤖 AGENT 2.2 (DATETIME TRANSLATION) - OUTPUT")
        logger.info("=" * 80)
        logger.info(f"📤 OUTPUT - datetime_range: {datetime_range}")
        logger.info(f"� OUTPUT - explanation: {explanation}")
        logger.info("=" * 80)
        print(f"🤖 DEBUG: ========== DATETIME AGENT OUTPUT ==========")
        ...
        if datetime_range == "none" or not datetime_range:
            logger.info("🗓️ RESULT: No temporal info → No datetime filter will be applied to STAC query")
            print("🗓️ DEBUG: No temporal info found - STAC query will use sortby=desc to get most recent")
            logger.info("=" * 80)
            print(f"🤖 DEBUG: ========== DATETIME AGENT END ==========")
            return None
        
        logger.info(f"✅ RESULT: Datetime filter will be applied to STAC query")
        logger.info(f"   ISO 8601 Range: {datetime_range}")
        logger.info(f"   Human Readable: {explanation}")
        logger.info("=" * 80)
        print(f"✅ DEBUG: DATETIME AGENT SUCCESS")
...
Return ONLY a JSON object with "cloud_intent", "threshold", and "reasoning". No additional text."""
        ...
        try:
            ...
            logger.info(f"🔧 AGENT 2.3: Using GPT-4o (temp=0.3, max_tokens=100)")
            ...
            if cloud_intent == "none" or not threshold:
                # 🔧 FALLBACK: Use keyword detection if GPT returns "none" but keywords exist
                ...
                if keyword_result:
                    ...
                    logger.info(f"🔧 FALLBACK: Keyword detection found cloud intent: {cloud_intent} ({threshold}%)")
                    print(f"🔧 DEBUG: Keyword fallback triggered - threshold: {threshold}%")
                else:
            ...
            print(f"❌ ERROR: JSON parse failed - {e}")
            # 🔧 FALLBACK: Try keyword detection on JSON parse failure
            keyword_result = await self._detect_cloud_cover_intent(query)
            if keyword_result and filterable:
                prop_name = get_cloud_cover_property(filterable[0])
                if prop_name:
                    logger.info(f"🔧 FALLBACK: Keyword detection after JSON error - threshold: {keyword_result['threshold']}%")
                    return {
            ...
            # 🔧 FALLBACK: Try keyword detection on any GPT failure
            keyword_result = await self._detect_cloud_cover_intent(query)
            if keyword_result and filterable:
                prop_name = get_cloud_cover_property(filterable[0])
                if prop_name:
                    logger.info(f"🔧 FALLBACK: Keyword detection after GPT error - threshold: {keyword_result['threshold']}%")
                    return {
                        "filter": {prop_name: {"lt": keyword_result["threshold"]}},
                        "threshold": keyword_result["threshold"],
    ...
    async def resolve_location_to_bbox(self, location_name: str, location_type: str = "region") -> Optional[List[float]]:
        ...
        logger.info(f"🏷️  Location Type: {location_type}")
        
        if not location_name:
            logger.warning("⚠️ No location name provided - skipping resolution")
            logger.info("=" * 80)
            return None
        ...
        try:
            ...
            if bbox:
                logger.info(f"✅ LOCATION RESOLVED SUCCESSFULLY")
                logger.info(f"📍 Location: {location_name}")
                logger.info(f"🗺️  BBox: [{bbox[0]:.4f}, {bbox[1]:.4f}, {bbox[2]:.4f}, {bbox[3]:.4f}]")
                logger.info(f"🗺️  Format: [west={bbox[0]:.4f}, south={bbox[1]:.4f}, east={bbox[2]:.4f}, north={bbox[3]:.4f}]")
                logger.info("=" * 80)
                # Cache the result for performance
                self.location_cache.set(location_name, location_type, bbox)
                return bbox
            else:
                logger.warning(f"❌ LOCATION RESOLUTION FAILED")
                logger.warning(f"📍 Location: {location_name}")
                logger.warning(f"🏷️  Type: {location_type}")
                logger.warning(f"ℹ️  All resolution strategies exhausted")
                logger.info("=" * 80)
                return None
                
        except Exception as e:
            logger.error(f"❌ LOCATION RESOLUTION ERROR")
            logger.error(f"📍 Location: {location_name}")
...
Location to analyze: {location_name}"""
    ...
    def select_collections(self, entities: Dict[str, Any]) -> List[str]:
        ...
        # 🔥 EXPLICIT DATASET NAME DETECTION (HIGHEST PRIORITY - before platform detection)
        ...
        if any(keyword in query_text for keyword in ["mtbs", "burn severity", "monitoring trends burn severity"]):
            logger.info(f"🔥 MTBS DATASET DETECTED in query: {query_text}")
            return ["mtbs"]
        
        if any(keyword in query_text for keyword in ["3dep", "lidar", "height above ground", "hag"]):
            logger.info(f"🏔️ 3DEP LIDAR DATASET DETECTED in query: {query_text}")
            return ["3dep-lidar-hag"]
        
        if any(keyword in query_text for keyword in ["usda", "cdl", "cropland data layer"]):
            logger.info(f"🌾 USDA CDL DATASET DETECTED in query: {query_text}")
            return ["usda-cdl"]
        
        if any(keyword in query_text for keyword in ["jrc", "global surface water"]):
            logger.info(f"🌊 JRC Global Surface Water DATASET DETECTED in query: {query_text}")
            return ["jrc-gsw"]
        
        if any(keyword in query_text for keyword in ["chloris", "biomass", "woody biomass"]):
            logger.info(f"🌲 Chloris Biomass DATASET DETECTED in query: {query_text}")
            return ["chloris-biomass"]
        
        if any(keyword in query_text for keyword in ["sea surface temperature", "sst", "ocean temperature"]):
            logger.info(f"🌊 Sea Surface Temperature DATASET DETECTED in query: {query_text}")
            return ["noaa-cdr-sea-surface-temperature-whoi"]
        
        if any(keyword in query_text for keyword in ["alos palsar", "palsar"]):
            logger.info(f"📡 ALOS PALSAR DATASET DETECTED in query: {query_text}")
            return ["alos-palsar-mosaic"]
        
        if any(keyword in query_text for keyword in ["buildings", "building footprints", "ms-buildings"]):
            logger.info(f"🏢 MS Buildings DATASET DETECTED in query: {query_text}")
            return ["ms-buildings"]
        
        if any(keyword in query_text for keyword in ["alos dem", "alos world 3d", "alos-dem"]):
            logger.info(f"🏔️ ALOS DEM DATASET DETECTED in query: {query_text}")
            return ["alos-dem"]
        
        # 🛰️ EXPLICIT SATELLITE PLATFORM DETECTION (SECOND PRIORITY)
        ...
        # 🌾 EXPLICIT DATASET NAME DETECTION (Add specific dataset keywords)
        if any(keyword in query_text for keyword in ["mtbs", "burn severity", "monitoring trends burn severity"]):
            logger.info(f"🔥 MTBS DATASET DETECTED in query: {query_text}")
            return ["mtbs"]
        ...
        if any(keyword in query_text for keyword in ["sea surface temperature", "sst", "ocean temperature"]):
            logger.info(f"🌡️ Sea Surface Temperature DATASET DETECTED in query: {query_text}")
            return ["noaa-cdr-sea-surface-temperature-whoi"]
        ...
        if any(keyword in query_text for keyword in ["buildings", "building footprints"]):
            logger.info(f"🏢 Building Footprints DATASET DETECTED in query: {query_text}")
            return ["ms-buildings"]
        
        if any(keyword in query_text for keyword in ["3dep", "lidar", "height above ground", "hag"]):
            logger.info(f"📡 USGS 3DEP Lidar DATASET DETECTED in query: {query_text}")
            return ["3dep-lidar-hag"]
        
        if any(keyword in query_text for keyword in ["alos world 3d", "alos dem"]) and "palsar" not in query_text:
            logger.info(f"🏔️ ALOS World 3D DATASET DETECTED in query: {query_text}")
            return ["alos-dem"]
        ...
        if "worldcover" in query_text or "esa world cover" in query_text:
            logger.info(f"🌍 ESA WorldCover DATASET DETECTED in query: {query_text}")
            return ["esa-worldcover"]
        
        if "cop-dem" in query_text or "copernicus dem" in query_text:
            logger.info(f"🏔️ Copernicus DEM DATASET DETECTED in query: {query_text}")
            return ["cop-dem-glo-30"]
        
        if "nasadem" in query_text:
            logger.info(f"🏔️ NASADEM DATASET DETECTED in query: {query_text}")
            return ["nasadem"]
        
        if "daymet" in query_text:
            logger.info(f"🌡️ Daymet DATASET DETECTED in query: {query_text}")
            return ["daymet-daily-na"]
        
        if "era5" in query_text:
            logger.info(f"🌡️ ERA5 DATASET DETECTED in query: {query_text}")
            if "land" in query_text:
                return ["era5-pds"]
            return ["era5-pds"]
        
        if any(keyword in query_text for keyword in ["viirs", "viirs fire"]):
            logger.info(f"🔥 VIIRS DATASET DETECTED in query: {query_text}")
            return ["modis-14A1-061"]
        
        if "io-lulc" in query_text or "io land cover" in query_text:
            logger.info(f"🌍 IO Land Cover DATASET DETECTED in query: {query_text}")
            return ["io-lulc-annual-v02"]
        
        if "lcmap" in query_text or "usgs land change" in query_text:
            logger.info(f"🌍 LCMAP DATASET DETECTED in query: {query_text}")
            return ["usgs-lcmap-conus-v13"]
        
        if "sentinel-5p" in query_text or "tropomi" in query_text:
            logger.info(f"🌬️ Sentinel-5P DATASET DETECTED in query: {query_text}")
            return ["sentinel-2-l2a"]
    ...
    def _get_dynamic_collections_by_category(self, category: str) -> Optional[List[str]]:
        ...
        if matching_collections:
            logger.info(f"✅ PC metadata found {len(matching_collections)} collections for {category}: {matching_collections[:5]}")
        ...
        if any(elev_word in query_text for elev_word in elevation_keywords):
            ...
            if any(lidar_word in query_text for lidar_word in lidar_keywords):
                logger.info(f"📡 LiDAR-specific request detected")
    ...
    async def translate_query(self, natural_query: str, pin_location: Optional[Dict[str, float]] = None, session_bbox: Optional[List[float]] = None, skip_intent_classification: bool = False, pre_classified_intent: Optional[str] = None) -> Dict[str, Any]:
        ...
        if session_bbox:
            logger.info(f"📍 Session bbox fallback available: {session_bbox}")
        logger.info("=" * 80)
        
        # 🔍 PIPELINE LOG: Query Input
        ...
        # ============================================================================
        # ⏱️ TIMING INSTRUMENTATION - Track query processing time
        # ============================================================================
        import time
        ...
        try:
            ...
            # 🔍 PIPELINE LOG: Classification Input
            ...
            # ============================================================================
            # 🚀 OPTIMIZATION: Skip classification if already done by UnifiedRouter
            # ============================================================================
            if skip_intent_classification and pre_classified_intent:
                logger.info(f"⚡ SKIP: Using pre-classified intent: {pre_classified_intent}")
                classification = {
            ...
            # 🔍 PIPELINE LOG: Classification & Collection Outputs
            ...
            logger.info(f"✅ PARALLEL PHASE 1 complete in {elapsed_parallel:.2f}s:")
            ...
            logger.info(f"⏱️ TIMING - Parallel Phase: {elapsed_parallel:.2f}s")
            ...
            # 🔍 PIPELINE LOG: STAC Query Building Input
            ...
            # 🔍 PIPELINE LOG: STAC Query Building Output
            ...
            logger.info(f"⏱️ TIMING - STAC Query Building: {elapsed_stac_build:.2f}s")
            ...
            # 🔍 PIPELINE LOG: Location Resolution
            ...
            # ========================================================================
            # 📍 SESSION_BBOX FALLBACK: Use session bbox when no location in query
            # ========================================================================
            ...
            if not bbox and session_bbox:
                logger.info(f"📍 FALLBACK: No location in query, using session bbox: {session_bbox}")
                bbox = session_bbox
                stac_query["bbox"] = bbox
            ...
            logger.info(f"⏱️ TOTAL TRANSLATION TIME: {elapsed_total:.2f}s")
            logger.info("=" * 80)
            return result
            ...
            if "LOCATION_REQUIRED" in str(ve):
                logger.warning(f"⚠️ Location required but not found in query: '{natural_query}'")
...
Keep your response focused, informative, and directly relevant to the user's question."""
    ...
    def _determine_tile_limit(self, bbox: List[float], query: str) -> int:
        ...
        logger.info(f"📏 _determine_tile_limit: area={area_km2:.0f} km² → base_limit={base_limit}")
