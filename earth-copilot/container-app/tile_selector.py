...
class TileSelector:
    ...
    def get_optimal_query_params(
        ...
        if user_datetime:
            ...
            logger.info(f"📅 User-specified datetime: {datetime_range}")
        else:
            ...
            if is_static_data:
                datetime_range = None
                logger.info(f"🏔️ Static dataset detected - no temporal filter needed")
            ...
            # Recent/Latest keywords → Last 30 days
                ...
                logger.info(f"🕒 'Recent' detected → Last 30 days: {datetime_range}")
            ...
            # Last week
                ...
                end_date = today.strftime("%Y-%m-%d")
                datetime_range = f"{start_date}/{end_date}"
                logger.info(f"🕒 'Last week' detected → Last 7 days: {datetime_range}")
            ...
            # Last month
                ...
                end_date = today.strftime("%Y-%m-%d")
                datetime_range = f"{start_date}/{end_date}"
                logger.info(f"🕒 'Last month' detected → Last 30 days: {datetime_range}")
            ...
            # Last year
                ...
                end_date = today.strftime("%Y-%m-%d")
                datetime_range = f"{start_date}/{end_date}"
                logger.info(f"🕒 'Last year' detected → Last 365 days: {datetime_range}")
            ...
            # Specific year (e.g., "2023 imagery")
                ...
                if year_match:
                    ...
                    logger.info(f"🕒 Specific year detected: {datetime_range}")
            ...
            # DEFAULT: Last 60 days (good balance of recency + availability)
                ...
                end_date = today.strftime("%Y-%m-%d")
                datetime_range = f"{start_date}/{end_date}"
                logger.info(f"🕒 No date specified → DEFAULT: Last 60 days: {datetime_range}")
                logger.info(f"   💡 This ensures recent, relevant data by default")
        ...
        if is_optical:
            ...
            if wants_clouds and not wants_clear:
                ...
                logger.info(f"☁️ User wants cloudy imagery - skipping cloud filter")
            else:
                ...
                if wants_clear or any(term in query_lower for term in ['clear', 'cloudless']):
                    cloud_threshold = 10
                    logger.info(f"☁️ 'Clear/low cloud' keyword detected → cloud threshold: <{cloud_threshold}%")
                
                quality_filters["eo:cloud_cover"] = {"lt": cloud_threshold}
                logger.info(f"☁️ Cloud cover filter: <{cloud_threshold}% (optical imagery)")
        
        # SAR imagery: No cloud filter needed (penetrates clouds)
        is_sar = any(col in collections for col in cls.SAR_COLLECTIONS)
        if is_sar:
            logger.info(f"📡 SAR imagery detected - no cloud filter needed")
        ...
        if is_usda_cdl:
            ...
            logger.info(f"🌾 USDA CDL: Filtering to 'cropland' type items (required for render config)")
        ...
        logger.info(f"✅ Optimal query params generated: limit={query_limit}, "
    ...
    def select_best_tiles(
        ...
        """
        if not features:
            logger.info("📭 No features to select from")
            return []
        
        logger.info(f"🎯 Selecting best tiles from {len(features)} candidates...")
        ...
        if query_bbox and len(query_bbox) == 4:
            ...
            if area_degrees > 10:
                ...
                logger.info(f"🌍 Large area detected ({area_degrees:.1f} sq deg) - skipping date grouping for full spatial coverage")
        ...
        if not skip_date_grouping:
            ...
            if tiles_by_date:
                ...
                if best_date_tiles:
                    logger.info(f"📅 Selected acquisition date: {best_date} ({len(best_date_tiles)} tiles)")
        ...
        if query_bbox:
            ...
            if bbox_area > 25:  # Country-scale (e.g., Greece ~71 sq deg)
                logger.info(f"🌍 Country-scale area ({bbox_area:.1f} sq deg) → using max_tiles={max_tiles}")
            elif bbox_area > 0.25:  # Large region
                logger.info(f"🗺️ Large area ({bbox_area:.1f} sq deg) → using max_tiles={max_tiles}")
        ...
        # =================================================================
        logger.info(f"✅ Selected {len(selected)} tiles (from {len(features)} candidates)")
        ...
        for i, tile in enumerate(selected[:3], 1):  # Log top 3
            ...
            logger.info(f"      📅 Date: {datetime_str}")
            logger.info(f"      ☁️ Clouds: {cloud_cover}%")
            logger.info(f"      📊 Scores: recency={breakdown['recency']:.1f}, "
    ...
    def _group_tiles_by_acquisition_date(cls, features: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        logger.info(f"📊 Grouped {len(features)} tiles into {len(tiles_by_date)} acquisition dates")
    ...
    def _select_best_date_group(
        ...
        best = date_scores[0]
        logger.info(f"📅 Best acquisition date: {best['date']} (score={best['score']:.1f}, tiles={len(best['tiles'])}, recency={best['recency']}, cloud={best['cloud']:.1f})")
    ...
    def _determine_scoring_weights(cls, query: Optional[str]) -> Dict[str, float]:
        ...
        if any(term in query_lower for term in [
            ...
            logger.info("🕐 User intent: MOST RECENT → Prioritizing recency (70%)")
            weights = {
        ...
        # PRIORITY 2: User explicitly wants CLEAR/CLOUDLESS
            ...
            logger.info("☀️ User intent: CLEAR IMAGERY → Prioritizing cloud cover (60%)")
            weights = {
                "recency": 15.0,
        ...
        # PRIORITY 3: User explicitly wants HIGH RESOLUTION
            ...
            logger.info("🔍 User intent: HIGH RESOLUTION → Prioritizing quality (50%)")
            weights = {
                "recency": 20.0,
                "cloud_cover": 20.0,
                "coverage": 10.0,
        ...
        # PRIORITY 4: User wants FULL COVERAGE
            ...
            logger.info("🗺️ User intent: FULL COVERAGE → Prioritizing coverage (50%)")
            weights = {
                "recency": 20.0,
                "cloud_cover": 15.0,
            ...
            logger.info("⚖️ User intent: BALANCED → Using default weights")
    ...
    def _calculate_optimal_tile_limit(
        ...
        if not bbox or len(bbox) != 4:
            logger.info(f"📍 No bbox provided → default limit: 100")
            return 100
        ...
        logger.info(f"📏 Query area dimensions:")
        ...
        if collections:
            ...
            for col in collections:
                ...
                if col in tile_sizes:
                    ...
                    if dynamic_size:
                        ...
                        logger.info(f"   📡 Dynamic query: {col} → {dynamic_size}km tiles")
                    else:
                        # Ultimate fallback
                        collection_tile_sizes.append(100)
                        logger.info(f"   ⚠️ Unknown collection {col}, using 100km default")
            ...
            logger.info(f"🗺️ Typical tile size for {collections}: {typical_tile_size_km}km × {typical_tile_size_km}km")
        else:
            logger.info(f"🗺️ Using default tile size: {typical_tile_size_km}km × {typical_tile_size_km}km")
        ...
        logger.info(f"🧮 Spatial calculation:")
        ...
        logger.info(f"🎯 Optimal tile limit calculation:")
        ...
        if final_limit == 50:
            logger.info(f"📍 TINY AREA: Using minimum limit (50)")
        elif final_limit < 150:
            logger.info(f"📍 SMALL AREA: Single tile or small mosaic")
        elif final_limit < 400:
            logger.info(f"🗺️ MEDIUM AREA: Multi-tile mosaic")
        elif final_limit < 800:
            logger.info(f"🗺️ LARGE AREA: Regional mosaic")
        else:
            logger.info(f"🗺️ VERY LARGE AREA: Using maximum limit (1000)")
