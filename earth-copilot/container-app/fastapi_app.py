...
async def try_alternative_queries(
    ...
    logger.info(f"📅 RELAXATION 2: Checking date range expansion for datetime='{datetime_str}'")
    if datetime_str and "/" in datetime_str:
        try:
            from datetime import datetime as dt, timedelta
            start, end = datetime_str.split("/")
            logger.info(f"📅 Parsed date range: start='{start}', end='{end}'")
            start_dt = dt.fromisoformat(start.replace("Z", ""))
            end_dt = dt.fromisoformat(end.replace("Z", ""))
            current_days = (end_dt - start_dt).days
            logger.info(f"📅 Current range is {current_days} days")
            ...
            for expand_days in [30, 60, 90]:
                ...
                logger.info(f"📅 Expanded datetime: {new_datetime}")
                
                alt_query = original_stac_query.copy()
                alt_query["datetime"] = new_datetime
                logger.info(f"📅 Alt query collections: {alt_query.get('collections')}, bbox: {alt_query.get('bbox')}")
                
                try:
                    alt_response = await execute_direct_stac_search(alt_query, stac_endpoint)
                    logger.info(f"📅 Alt response success: {alt_response.get('success')}, features: {len(alt_response.get('results', {}).get('features', []))}")
                    ...
                    logger.warning(f"⚠️ Date expansion failed for {expand_days} days: {type(e).__name__}: {e}")
                    import traceback
                    logger.warning(f"⚠️ Traceback: {traceback.format_exc()}")
                    continue
        except Exception as e:
            logger.warning(f"⚠️ Date parsing failed: {type(e).__name__}: {e}")
            import traceback
            logger.warning(f"⚠️ Traceback: {traceback.format_exc()}")
...
async def health_check():
    ...
    try:
        ...
        available_models_str = os.getenv("AZURE_OPENAI_AVAILABLE_MODELS", "gpt-4o")
        available_models = [m.strip() for m in available_models_str.split(",") if m.strip()] if available_models_str else ["gpt-4o"]
        ...
        basic_status = {
            ...
            "azure_openai_deployment": bool(deployment_name),  # Always true since we have a default
...
async def unified_query_processor(request: Request):
    ...
    try:
        ...
        if semantic_translator:
            ...
            logger.warning("⚠️ semantic_translator is None - AI functionality disabled")
                ...
                try:
                    ...
                    if quickstart_data:
                        ...
                        if temporal:
                            ...
                            logger.info(f"📅 Converted temporal '{temporal}' to datetime range: {datetime_range}")
            ...
            try:
                ...
                if stac_response.get("success"):
                    ...
                    if search_diagnostics["final_count"] == 0:
                        ...
                        logger.info(f"🔄 STAC query datetime: {stac_query.get('datetime')}, collections: {stac_query.get('collections')}, bbox: {stac_query.get('bbox')}")
                        try:
                            ...
                            logger.info(f"🔄 Alternative result: success={alternative_result.get('success')}, features={len(alternative_result.get('features', []))}")
                        except Exception as alt_ex:
                            logger.error(f"🔄 EXCEPTION in try_alternative_queries: {type(alt_ex).__name__}: {alt_ex}")
                            import traceback
...
async def geoint_vision_analysis(request: Request):
    ...
    try:
        ...
        # 🎯 NEW: Accept analysis_type hint from frontend (raster vs screenshot)
        ...
        if analysis_type:
            logger.info(f"🎯 [{request_id}] Analysis type hint: {analysis_type}")
        ...
        logger.info(f"👁️ [{request_id}] Pin coordinates set: ({map_bounds.get('pin_lat')}, {map_bounds.get('pin_lng')})")
        ...
        try:
            result = await asyncio.wait_for(
                vision_agent.analyze(
                    ...
                    analysis_type=analysis_type  # 🎯 Pass hint from frontend
                ),
                timeout=240.0
            )
        except asyncio.TimeoutError:
...
async def geoint_vision_chat(request: Request):
    ...
    try:
        ...
        # 🎯 NEW: Accept analysis_type hint from frontend
        analysis_type = request_data.get("analysis_type")
        if analysis_type:
            logger.info(f"🎯 [{request_id}] Analysis type hint from frontend: {analysis_type}")
        ...
        if latitude and longitude:
            ...
            logger.info(f"💬 [{request_id}] Pin coordinates set: ({latitude}, {longitude})")
