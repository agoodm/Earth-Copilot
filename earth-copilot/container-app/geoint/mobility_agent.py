...
class GeointMobilityAgent:
    ...
    async def analyze_mobility(
        ...
        # Perform directional mobility analysis (algorithmic) - now async with raster sampling
        mobility_assessment = await self._assess_directional_mobility(
    ...
    async def _read_cog_window(
        ...
        """
        def _read_sync():
            """Synchronous rasterio read - runs in thread pool."""
            try:
                import rasterio
        ...
        try:
            # Run synchronous rasterio in thread pool to avoid blocking event loop
            loop = asyncio.get_event_loop()
            result = await loop.run_in_executor(None, _read_sync)
            return result
        except Exception as e:
            logger.error(f"COG read executor failed: {e}")
            return None
    ...
    async def _assess_directional_mobility(
        self,
        latitude: float,
        longitude: float,
        ...
        # Analyze all four directions concurrently
        north, south, east, west = await asyncio.gather(
            self._analyze_direction("North", latitude, longitude, terrain_data, "N"),
            self._analyze_direction("South", latitude, longitude, terrain_data, "S"),
            self._analyze_direction("East", latitude, longitude, terrain_data, "E"),
            self._analyze_direction("West", latitude, longitude, terrain_data, "W")
        )
        ...
        if terrain_data.get("active_fires") and terrain_data["active_fires"].get("features"):
            try:
                ...
                if fire_mask_url:
                    ...
                    fire_pixels = await self._read_cog_window(fire_mask_url, direction_bbox, band=1)
        ...
        if status != "NO-GO" and terrain_data.get("water_detection") and terrain_data["water_detection"].get("features"):
            try:
                ...
                if vv_asset_url:
                    ...
                    sar_pixels = await self._read_cog_window(vv_asset_url, direction_bbox, band=1)
        ...
        if status == "GO" and terrain_data.get("elevation_profile") and terrain_data["elevation_profile"].get("features"):
            try:
                ...
                if dem_asset_url:
                    ...
                    dem_pixels = await self._read_cog_window(dem_asset_url, direction_bbox, band=1)
        ...
        if status == "GO" and terrain_data.get("vegetation_density") and terrain_data["vegetation_density"].get("features"):
            try:
                ...
                if red_asset_url and nir_asset_url:
                    ...
                    red_pixels = await self._read_cog_window(red_asset_url, direction_bbox, band=1)
                    nir_pixels = await self._read_cog_window(nir_asset_url, direction_bbox, band=1)
