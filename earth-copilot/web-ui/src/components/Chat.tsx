    ...
    const handleStacQueryEvent = (event: CustomEvent<{ query: string; clearSessions: boolean }>) => {
      ...
      if (query && query.trim()) {
        ...
        if (event.detail.clearSessions && localVisionSessionId) {
          console.log('🗑️ Chat: Clearing local vision session for new STAC search');
          setLocalVisionSessionId(null);
        }
        ...
        if (event.detail.clearSessions) {
          if (onClearVisionSession) {
            console.log('🗑️ Chat: Calling onClearVisionSession callback');
            onClearVisionSession();
          }
          if (onClearTerrainSession) {
            console.log('🗑️ Chat: Calling onClearTerrainSession callback');
            onClearTerrainSession();
          }
        }
  ...
  useEffect(() => {
    const handleQueryEvent = (event: CustomEvent<{ 
      ...
      if (query && query.trim()) {
        ...
        console.log('🔍 Chat: Current state (from refs) - selectedModule:', currentSelectedModule, 'vision_pin:', currentMapContext?.vision_pin);
        ...
        // Check if Vision module is ON (selectedModule === 'vision')
        if (requiresVision && currentSelectedModule !== 'vision') {
          console.warn('⚠️ Chat: Vision validation failed - selectedModule is:', currentSelectedModule, 'but expected "vision"');
          validationErrors.push('**Vision module is not active.** Please click the 👁️ Vision button in the module panel first.');
        }
        ...
        if (validationErrors.length > 0) {
          console.warn('⚠️ Chat: Vision query validation failed:', validationErrors);
          const errorMessage: ChatMessage = {
            role: 'assistant',
            content: `⚠️ **Cannot run Vision analysis yet**\n\nPlease complete these steps first:\n\n${validationErrors.map((e, i) => `${i + 1}. ${e}`).join('\n\n')}`,
            timestamp: new Date()
          };
  ...
  const chatMutation = useMutation({
    mutationFn: async (message: string) => {
      try {
        ...
        const result = await apiService.sendChatMessage(message, selectedDataset?.id, conversationId, messages, currentPin || undefined, geointMode, mapContext, selectedModel);
