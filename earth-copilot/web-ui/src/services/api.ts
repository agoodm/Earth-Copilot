...
class ApiService {
  ...
  async clearTerrainSession(sessionId: string): Promise<boolean> {
    console.log('🗑️ API: Clearing terrain session', sessionId);
  ...
  async sendVisionChatMessage(
    ...
    try {
      ...
      if (!sessionId) {
        console.log('👁️ API: First vision message - using /api/geoint/vision to create session');
        ...
        if (mapContext?.tile_urls?.length > 0) {
          requestData.tile_urls = mapContext.tile_urls;
          console.log('🗺️ Including', mapContext.tile_urls.length, 'tile URLs for raster analysis');
        }
        if (mapContext?.current_collection) {
      ...
      if (mapContext?.stac_items?.length > 0) {
        ...
        console.log('⚠️ [FOLLOW-UP] No STAC items in mapContext! stac_items:', mapContext?.stac_items);
      }
  ...
  async clearVisionSession(sessionId: string): Promise<boolean> {
    console.log('🗑️ API: Clearing vision session', sessionId);
  ...
  async triggerGeointAnalysis(
    ...
    try {
      ...
      // Add screenshot for vision and terrain analysis
      if ((module === 'vision' || module === 'terrain') && screenshot) {
        requestData.screenshot = screenshot;
